      ...
      {session && session.user ? (
        <Link href="/profile" className={`${path === "profile" && "active"}`} aria-label="Edit your profile">
          <Image
            ...
            className={`rounded-full h-5 w-5 object-cover avatar ${path === "profile" && "border-2 border-accent"}`}
            height={5}
            width={5}
            alt="PFP"
            priority
        ...
        <h1 className="text-xl font-bold flex justify-center items-center" aria-label="Sign in or create an account">
