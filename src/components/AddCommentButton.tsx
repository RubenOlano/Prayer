...
function AddCommentButton({ postId }: Props) {
  ...
  const onSubmit = (data: CreateCommentSchema) => mutate({ ...data, postId });
  ...
  return (
    <div className="flex flex-col items-center justify-center py-2 p-3 m-2 rounded-lg md:max-w-[65vw]">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col items-center justify-center">
        <textarea placeholder="Comment" className="border-2 border-black rounded-md p-2 m-2" {...register("content")} />
        ...
        <div className="flex flex-col items-center justify-center">
          {errors.content && <p className="text-red-500">{errors.content.message || "Something went wrong"}</p>}
        </div>
      </form>
    </div>
  );
