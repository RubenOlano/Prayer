...
export const groupRouter = router({
  ...
  fetchUserIsAdmin: protectedProcedure.input(fetchUserIsAdminSchema).query(async ({ ctx, input }) => {
    ...
    const userId = ctx.session.user.id;
    try {
      const user = await ctx.prisma.user.findUniqueOrThrow({ where: { id: userId }, include: { GroupAdmins: true } });
      const isAdmin = user.GroupAdmins.find(admin => admin.groupId === groupId);
      return !!isAdmin;
    } catch (error) {
      throw handleError(error);
    }
  ...
  getExploreGroups: protectedProcedure.query(async ({ ctx }) => {
    ...
    try {
      ...
      const groups = await ctx.prisma.group.findMany({
        where: {
          NOT: {
            OR: [{ GroupMembers: { some: { userId } } }, { GroupAdmins: { some: { userId } } }, { private: true }],
          },
        },
        include: {
